...
const UserManagement = () => {
    ...
    if (isLoading && !data) return (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-6">
            ...
            <p className="font-mono text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse">Scanning Personnel Database...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-slate-100 pb-10">
                <div>
                    <h1 className="text-4xl font-heading font-black text-navy-900 uppercase tracking-tighter">Personnel Registry</h1>
                    <div className="flex items-center gap-3 mt-2">
                        <div className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                        <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">Authorized Records: {data?.pagination.total || 0} Entities Found</p>
                    </div>
                </div>
                <div className="flex flex-wrap items-center gap-4">
                    <button className="flex items-center gap-3 px-6 h-14 rounded-2xl bg-white border border-slate-100 text-slate-400 hover:text-navy-900 hover:border-emerald-200 transition-all font-heading font-black uppercase text-[10px] tracking-widest shadow-sm group">
                        <Download size={16} />
                        Export Registry
                    </button>
                    <button className="flex items-center gap-3 px-8 h-14 rounded-2xl bg-navy-900 text-white hover:bg-emerald-600 transition-all font-heading font-black uppercase text-[10px] tracking-widest shadow-xl shadow-navy-900/10 active:scale-95">
                        <UserPlus size={16} />
            ...
            <div className="p-6 rounded-[2.5rem] bg-white border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.01)] flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="bg-white border border-slate-100 rounded-[3rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.01)]">
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100">
                                <th className="px-10 py-8 text-[10px] font-mono font-black text-slate-400 uppercase tracking-[0.3em]">Identity Node</th>
                                <th className="px-10 py-8 text-[10px] font-mono font-black text-slate-400 uppercase tracking-[0.3em]">Access Level</th>
                                <th className="px-10 py-8 text-[10px] font-mono font-black text-slate-400 uppercase tracking-[0.3em]">Enrolled On</th>
                                <th className="px-10 py-8 text-[10px] font-mono font-black text-slate-400 uppercase tracking-[0.3em]">Security Pulse</th>
                                <th className="px-10 py-8 text-[10px] font-mono font-black text-slate-400 uppercase tracking-[0.3em] text-right">Operations</th>
                            </tr>
                        </thead>
                                    ...
                                    >
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-5">
                                                <div className="w-12 h-12 rounded-xl bg-navy-900 flex items-center justify-center text-emerald-400 text-lg font-black shadow-lg group-hover:scale-105 transition-transform duration-500">
                                                    {user.name[0]?.toUpperCase()}
                                                </div>
                                                <div className="min-w-0">
                                                    <p className="text-[14px] font-heading font-black text-navy-900 uppercase tracking-tight leading-none mb-1.5">{user.name}</p>
                                                    <div className="flex items-center gap-2 text-slate-400">
                                                        <Mail size={12} />
                                                        <span className="text-[10px] font-mono font-bold tracking-tight">{user.email}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-2 text-slate-500">
                                                ...
                                                <span className="text-[10px] font-mono font-bold uppercase tracking-widest">{new Date(user.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-6">
                                            <div className="flex items-center gap-3">
                                                <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)] group-hover:animate-ping" />
                                                <span className="text-[10px] font-mono font-black text-slate-500 uppercase tracking-widest">Linked</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                ...
                                                <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-slate-100 text-slate-200 hover:text-navy-900 hover:border-slate-300 transition-all shadow-sm">
