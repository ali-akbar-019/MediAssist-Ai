...
const AdminDashboard = () => {
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-6" data-testid="admin-loading">
            <div className="w-16 h-16 border-4 border-emerald-100 border-t-emerald-500 rounded-full animate-spin shadow-sm" data-testid="admin-loading-spinner" />
            <p className="font-mono text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse" data-testid="admin-loading-text">Syncing Terminal Logic...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-10 pb-20" data-testid="admin-dashboard">
            {/* Header Sector */}
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10" data-testid="admin-header">
                <div>
                    <h1 className="text-4xl font-heading font-black text-navy-900 uppercase tracking-tighter" data-testid="admin-heading">Command Center</h1>
                    <div className="flex items-center gap-3 mt-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                        <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-[0.3em]" data-testid="admin-status">Operational Readiness: Optimal</p>
                    </div>
                </div>
                <div className="flex items-center gap-4" data-testid="admin-latency">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8" data-testid="admin-console">

                {/* Sector 1: Intelligence Accuracy */}
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)] relative overflow-hidden group" data-testid="admin-intelligence">
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight" data-testid="admin-intelligence-title">Intelligence</h3>
                            <p className="text-[9px] font-mono font-bold text-slate-400 uppercase">Verification Loop</p>
                        </div>
                    </div>

                    <div className="h-48 bg-slate-50/50 rounded-[2rem] border border-slate-100 flex items-end p-6 gap-2 relative overflow-hidden mb-8" data-testid="admin-intelligence-chart">
                        {[40, 70, 45, 90, 60, 85, 50, 95, 75, 40, 80].map((h, i) => (
                            <motion.div
                                key={i}
                                data-testid={`admin-chart-bar-${i}`}
                                initial={{ height: 0 }}
                                animate={{ height: `${h}%` }}
                    ...
                    <div className="space-y-4">
                        <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100" data-testid="admin-confidence">
                            ...
                            <p className="text-2xl font-heading font-black text-emerald-600" data-testid="admin-confidence-value">{stats?.ai.accuracyRate}</p>
                        </div>
                        <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100" data-testid="admin-most-logged">
                            ...
                            <p className="text-lg font-heading font-black text-navy-900 uppercase truncate" data-testid="admin-most-logged-value">{stats?.ai.mostLognedSymptom || "N/A"}</p>
                        </div>
                    </div>
                </section>

                {/* Sector 2: Operational Threads */}
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)]" data-testid="admin-operations">
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight" data-testid="admin-operations-title">Active Logic</h3>
                            <p className="text-[9px] font-mono font-bold text-slate-400 uppercase">Processing Pipelines</p>
                        </div>
                    </div>

                    <div className="flex-1 space-y-8" data-testid="admin-threads">
                        {[
                            ...
                            <div key={i} data-testid={`admin-thread-${i}`}>
                                <div className="flex justify-between items-center mb-2 px-1">
                                    <span className="text-[10px] font-heading font-bold text-navy-900 uppercase tracking-tight" data-testid={`admin-thread-label-${i}`}>{thread.label}</span>
                                    <span className="text-[9px] font-mono font-black text-slate-400" data-testid={`admin-thread-progress-${i}`}>{thread.progress}%</span>
                                </div>
                                <div className="h-3 bg-slate-50 rounded-full border border-slate-100 p-0.5">
                                    <motion.div
                                        ...
                                        data-testid={`admin-thread-bar-${i}`}
                                    />
                                </div>
                            </div>
                        ))}
                ...
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)] min-h-[500px]" data-testid="admin-pulse">
                    <div className="flex items-center gap-4 mb-10">
                        <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-100">
                            <TrendingUp className="text-navy-900" size={22} />
                        </div>
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight" data-testid="admin-pulse-title">System Pulse</h3>
                    ...
                    <div className="flex-1 space-y-4" data-testid="admin-pulse-events">
                        {[
                            ...
                            <div key={i} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-all group cursor-pointer border border-transparent hover:border-slate-100" data-testid={`admin-pulse-event-${i}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[12px] font-heading font-black text-navy-900 uppercase tracking-tight truncate" data-testid={`admin-pulse-event-label-${i}`}>{event.label}</p>
                                    <div className="flex items-center gap-2 text-slate-400">
                                        <Clock size={10} />
                                        <span className="text-[9px] font-mono font-bold uppercase" data-testid={`admin-pulse-event-time-${i}`}>{event.time}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <button
                        data-testid="admin-initialize-btn"
