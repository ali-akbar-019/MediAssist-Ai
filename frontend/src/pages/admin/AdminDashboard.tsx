...
const AdminDashboard = () => {
    ...
    if (isLoading) return (
        <div className="flex flex-col items-center justify-center h-[60vh] gap-6">
            <div className="w-16 h-16 border-4 border-emerald-100 border-t-emerald-500 rounded-full animate-spin shadow-sm" />
            <p className="font-mono text-[10px] font-black text-slate-400 uppercase tracking-widest animate-pulse">Syncing Terminal Logic...</p>
        </div>
    );
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
                <div>
                    <h1 className="text-4xl font-heading font-black text-navy-900 uppercase tracking-tighter">Command Center</h1>
                    <div className="flex items-center gap-3 mt-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                        <p className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-[0.3em]">Operational Readiness: Optimal</p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)] relative overflow-hidden group">
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight">Intelligence</h3>
                            <p className="text-[9px] font-mono font-bold text-slate-400 uppercase">Verification Loop</p>
                        </div>
                    </div>

                    <div className="h-48 bg-slate-50/50 rounded-[2rem] border border-slate-100 flex items-end p-6 gap-2 relative overflow-hidden mb-8">
                    ...
                    <div className="space-y-4">
                        <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100">
                            ...
                            <p className="text-2xl font-heading font-black text-emerald-600">{stats?.ai.accuracyRate}</p>
                        </div>
                        <div className="p-5 rounded-2xl bg-slate-50 border border-slate-100">
                            <p className="text-[9px] font-mono font-black text-slate-400 uppercase tracking-widest mb-1">Most Logged Hub</p>
                            <p className="text-lg font-heading font-black text-navy-900 uppercase truncate">{stats?.ai.mostLognedSymptom || "N/A"}</p>
                        </div>
                    </div>
                </section>
                ...
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)]">
                    <div className="flex items-center gap-4 mb-10">
                        ...
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight">Active Logic</h3>
                        ...
                        {[
                            ...
                            <div key={i}>
                                <div className="flex justify-between items-center mb-2 px-1">
                                    <span className="text-[10px] font-heading font-bold text-navy-900 uppercase tracking-tight">{thread.label}</span>
                                    <span className="text-[9px] font-mono font-black text-slate-400">{thread.progress}%</span>
                                </div>
                ...
                <section className="bg-white rounded-[3rem] border border-slate-100 p-8 flex flex-col shadow-[0_20px_60px_rgba(0,0,0,0.01)] min-h-[500px]">
                    <div className="flex items-center gap-4 mb-10">
                        <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-100">
                            <TrendingUp className="text-navy-900" size={22} />
                        </div>
                        <div>
                            <h3 className="text-lg font-heading font-black text-navy-900 uppercase tracking-tight">System Pulse</h3>
                        ...
                        {[
                            ...
                            <div key={i} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-all group cursor-pointer border border-transparent hover:border-slate-100">
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-[12px] font-heading font-black text-navy-900 uppercase tracking-tight truncate">{event.label}</p>
                                    <div className="flex items-center gap-2 text-slate-400">
                                        <Clock size={10} />
                                        <span className="text-[9px] font-mono font-bold uppercase">{event.time}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <button className="mt-10 py-5 w-full rounded-2xl bg-navy-900 hover:bg-emerald-600 text-white font-heading font-black uppercase text-[10px] tracking-[0.3em] transition-all duration-500 shadow-xl shadow-navy-900/10 active:scale-95 flex items-center justify-center gap-3">
                        <Cpu size={16} />
                        Initialize Terminal
                    </button>
                </section>
